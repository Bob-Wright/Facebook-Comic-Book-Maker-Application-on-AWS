...
public function checkUser($userData = array()){
	if(!empty($userData)){
		...
		if($prevResult->num_rows > 0){
			...
			// Insert user data
			$query = "INSERT INTO ".$this->userTbl." ( oauth_provider, oauth_id, name, first_name, last_name, email, picture, link, gallery_name, created, modified) VALUES ('".$userData['oauth_provider']."', '".$userData['oauth_id']."',  '".$userData['name']."', '".$userData['first_name']."', '".$userData['last_name']."', '".$userData['email']."', '".$userData['picture']."', '".$userData['link']."', 'none', NOW(), NOW())";
			//$query = '"'.$query.'"';
			//echo $query."<br>";
